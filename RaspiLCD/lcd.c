...
//--------------------------------------------------------------------------------------------------
// Name:      getPxLength
// Function:  get pixel length of string
//            
// Parameter: string
// Return:    pixel length
//--------------------------------------------------------------------------------------------------
...
// Return:    -
//--------------------------------------------------------------------------------------------------
void LCD_PrintXY(int x0,int y0,char *s)
{
	uint8	ix,iy,y;
	const uint8 *font;
	const uint8 *pt;
...
void LCD_PrintXYVz(int x0,int y0,char *s)
{
	uint8	ix,iy,y;
	const uint8 *font;
	const uint8 *pt;
		...
		while(i_char_height)
		{
			for(ix=0;ix<char_width;ix++)   
			{
				...
				for(iy=0;iy<8;iy++)	// je ein Byte vertikal ausgeben
				{
					if(d & (1<<iy))
					{
						if(x0+ix > 7 && x0+ix < 127)
						{
							LCD_PutPixel(x0+ix,y+iy,0);
						}
					}else{
						if(x0+ix > 7 && x0+ix < 127)
						{
							LCD_PutPixel(x0+ix,y+iy,1);
						}
					}	
				}
			}
...
void LCD_PrintXYWhite(int x0,int y0,char *s)
{
	uint8	ix,iy,y;
	const uint8 *font;
	const uint8 *pt;
	...
	while(*s)
	{
		...
		while(i_char_height)
		{
			for(ix=0;ix<char_width;ix++)   
			{
				...
				for(iy=0;iy<8;iy++)	// je ein Byte vertikal ausgeben
				{
					if(d & (1<<iy))	LCD_PutPixel(x0+ix,y+iy,0);	
						else			LCD_PutPixel(x0+ix,y+iy,1);	
				}
			}
			i_char_height = (i_char_height >= 8) ? i_char_height - 8 : 0; 
			y+=8;		// nächste "Page"
